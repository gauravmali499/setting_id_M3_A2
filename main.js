var productList=[
  ...
  function ListofProduct(){
    ...
    for (let i = 0; i < productList.length; i++) {
      ...
      row.insertCell(0).innerHTML = `<div>
          ...
          <img src="${productList[i].image}" alt="${productList[i].title}" style="width: 140px; height: 180px"> 
  ...
  function addToCart(productId){    
      if (cartList.includes(Number(productList[productId-1].id))) {
          ...
          row.insertCell(1).innerHTML = productList[productId-1].title;
          row.insertCell(2).innerHTML = `<p>${productList[productId-1].price}</p>`;
  ...
  function calculateTotalAmount() {
    ...
    for (var i = 0; i < cartList.length; i++) {
        var productId = cartList[i];
        var quantity = Number(document.getElementById(`txtQty${productId}`).value);
  ...
  function updateTotalAmountOnPage() {
    var total_amount = document.getElementById("totalAmount");
    total_amount.innerHTML = `Total Bill is : ${amount}`;
  }
  ...
  function removeItem(productId){
    var quantity = Number(document.getElementById(`txtQty${productId}`).value);
    amount = amount - (quantity * Number(productList[productId - 1].price));

    var table1 = document.getElementById("table1");
    var rowIndex = -1;
    for (var i = 0; i < table1.rows.length; i++) {
        if (table1.rows[i].cells[0].innerHTML == productList[productId - 1].id) {
            rowIndex = i;
            break;
        }
    }
    ...
    cartList = cartList.filter(function (e) {
        return e !== productList[productId - 1].id;
    });

    console.log(cartList);
    var total_amount = document.getElementById("totalAmount");
    total_amount.innerHTML = `Total Bill is : ${amount}`;
    document.getElementById(`btnCart${productList[productId-1].id}`).disabled = false;
  }
